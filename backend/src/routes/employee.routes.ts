...
import { isLevelAllowedForPosition } from '../services/orgStructure.service';
...
router.post('/', asyncHandler(async (req, res) => {
  ...
  if (position_id && level_id && !(await isLevelAllowedForPosition(Number(position_id), Number(level_id)))) {
    res.status(400).json({ error: 'ระดับที่เลือกไม่ตรงกับตำแหน่งนี้' });
    return;
  }
